...
let AnalyticsService = class AnalyticsService {
    ...
    async getDashboardStats() {
        ...
        const [totalEvents, todayPageViews, todayLogins, weeklyDownloads, weeklyJobClicks, monthlySignups, monthlyPayments, eventBreakdown, dailyPageViews, departmentEngagement, totalRevenueData, monthlyRevenueData, dailyRevenueData,] = await Promise.all([
            ...
            this.analyticsModel.countDocuments({ event: analytics_schema_1.EventType.DOWNLOAD, createdAt: { $gte: sevenDaysAgo } }).exec(),
            this.analyticsModel.countDocuments({ event: analytics_schema_1.EventType.JOB_CLICK, createdAt: { $gte: sevenDaysAgo } }).exec(),
            this.analyticsModel.countDocuments({ event: analytics_schema_1.EventType.SIGNUP, createdAt: { $gte: thirtyDaysAgo } }).exec(),
            this.analyticsModel.countDocuments({ event: analytics_schema_1.EventType.PAYMENT_SUCCESS, createdAt: { $gte: thirtyDaysAgo } }).exec(),
            ...
            this.analyticsModel.aggregate([
                { $match: { event: analytics_schema_1.EventType.PAGE_VIEW, createdAt: { $gte: new Date(now.getTime() - 14 * 24 * 60 * 60 * 1000) } } },
                { $group: {
